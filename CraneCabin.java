...
public class CraneCabin implements AnalogListener{
    ...
    public CraneCabin(Spatial craneSpatial){
        ...
        createCranePhysics();
        System.out.println(((Node)mobileCrane.getChild("longCraneElement")).getLocalScale());
        System.out.println(((RigidBodyControl)((Node)mobileCrane.getChild("longCraneElement")).getControl(0)).getCollisionShape().getScale());
    }
    public void onAnalog(String name, float value, float tpf) {
        switch(name){
            case "Right":
                craneCabin.rotate(0, -tpf, 0);
                ((RigidBodyControl)(craneCabin.getControl(0))).setPhysicsRotation(craneCabin.getLocalRotation());
                break;
            case "Left": 
                craneCabin.rotate(0, tpf, 0);
                break;
           case "Up":
            ...
            case "Pull out":
                ...
                /*Geometry g1 = ((Geometry)mobileCrane.getChild("Cube.0071"));
                g1.getLocalScale().z += 0.1f;
                mobileCrane.getChild("hookHandle").getLocalTranslation().z += 0.1f;
    ...
    private void createCranePhysics(){
        ...
        CollisionShape c1 = CollisionShapeFactory.createDynamicMeshShape(g1);
        c1.setScale(g1.getWorldScale());
        cabinCollision.addChildShape(c1, new Vector3f(0,0,0)
                .subtract(craneCabin.getWorldTranslation())
                .add(g1.getWorldTranslation()), g1.getWorldRotation().toRotationMatrix());
        ...
        cabinCollision.addChildShape(c2, new Vector3f(0,0,0)
                .subtract(craneCabin.getWorldTranslation())
                .add(g2.getWorldTranslation()), g2.getWorldRotation().toRotationMatrix());
        
        RigidBodyControl cabinControl = new RigidBodyControl(cabinCollision, 1f);
        cabinControl.setLinearVelocity(new Vector3f(1,1,1));
        cabinControl.setAngularFactor(0);
        cabinControl.setKinematic(false);
        ...
        RigidBodyControl rgc2 = new RigidBodyControl(com2, 1f);
